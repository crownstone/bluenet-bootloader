    ...
    switch (packet)
    {
        ...
        case DATA_PACKET:
            if (result != NRF_SUCCESS)
            {
                ...
                if (IS_CONNECTED())
                {
WRITE_VERBOSE("cb handler disconnect\r\n", 23);
...
//            nrf_gpio_pin_set(ADVERTISING_LED_PIN_NO);
    ...
    WRITE_VERBOSE("\r\n", 3);
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            {
WRITE_VERBOSE("ble evt disconnected\r\n", 22);
...
//                keys.keys_central.p_enc_key = &enc_key;
        ...
        case BLE_GATTS_EVT_TIMEOUT:
            ...
            {
WRITE_VERBOSE("timeout disconnect\r\n", 20);
                err_code = sd_ble_gap_disconnect(m_conn_handle,
                                                 BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
                APP_ERROR_CHECK(err_code);
            }
...
uint32_t dfu_transport_close()
{
    ...
    if (IS_CONNECTED())
    {
WRITE_VERBOSE("transport close disconnect\r\n", 30);
        // Disconnect from peer.
